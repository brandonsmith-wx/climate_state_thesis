...
for CASENAME in casenames:
    ...
    if not os.path.isfile(outpath+outfile_control):
        if os.path.isfile(inpath+infile):
            ...
            print(syscall)
            #syscall = 'cdo mastrfu '+outpath+outfile_control+' '+outpath+outfile_control_stf
            #os.system(syscall)
    run = '2xCO2'
    inpath = '/home/brandonsmith/modeloutput/'+run+'/'+CASENAME
...
for CASENAME in casenames:
    ...
    if os.path.isfile(dsloc_control) and os.path.isfile(dsloc_2xco2):
        ...
        for j in range(0,len(lev)):
            if j < 29:
                ccn = (R/g)*CCN[j]*np.log(lev[j+1]/lev[j])*(T[j+1]-T[j])
                ccnd = (R/g)*CCNd[j]*np.log(lev[j+1]/lev[j])*(Td[j+1]-Td[j])
                ccnq = (R/g)*CCNq[j]*np.log(lev[j+1]/lev[j])*(Tq[j+1]-Tq[j])
                ccn1 = (R/g)*CCN1[j]*np.log(lev[j+1]/lev[j])*(T1[j+1]-T1[j])
                ccn1d = (R/g)*CCN1d[j]*np.log(lev[j+1]/lev[j])*(T1d[j+1]-T1d[j])
                ccn1q = (R/g)*CCN1q[j]*np.log(lev[j+1]/lev[j])*(T1q[j+1]-T1q[j])
...
#ax.legend(labels=['Tropics (0$^o$ - 30$^o$)', 'Mid Latitudes (30$^o$ - 60$^o$)', 'Poles (60$^o$ - 90$^o$)'])
plt.grid(True)
