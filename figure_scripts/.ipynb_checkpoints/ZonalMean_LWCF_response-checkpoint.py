...
#
# Script for plotting figure 3.18. for altercation, make necessary comments and uncomments
...
for CASENAME in casenames:
    ...
    outfile_control = '/ZonalMean_LWCF_'+run+'_'+CASENAME+'.nc'
    ...
    outpath = '/home/brandonsmith/modeloutput/'+run+'/'+CASENAME
    outfile_2xCO2 = '/ZonalMean_LWCF_'+run+'_'+CASENAME+'.nc'
    ...
    infile = '/Merged_'+run+'_'+CASENAME+'_.nc'
    outpath = '/home/brandonsmith/modeloutput/'+run+'/'+CASENAME
    outfile_4xCO2 = '/ZonalMean_LWCF_'+run+'_'+CASENAME+'.nc'
...
for CASENAME in casenames:
    ...
    outfile_control = '/ZonalMean_LWCF_Control_'+CASENAME+'.nc'
    outfile_2xCO2 = '/ZonalMean_LWCF_2xCO2_'+CASENAME+'.nc'
    outfile_4xCO2 = '/ZonalMean_LWCF_4xCO2_'+CASENAME+'.nc'
    ...
    yticks = np.linspace(0,50,6).astype(int)
    ydiffticks = np.linspace(-15,15,7).astype(int)
    ax.set_xlim(-1,1)
    ax.set_yticks(yticks)
    ax.set_yticklabels(yticks,fontsize=12)

    if i == 0:
        ax.set_title('Control Climates',fontsize=12)
    ax.set_ylabel('LWCF ($W/m^2$)',fontsize=12)
    ax.set_xticks(ticks)
    ...
    ax2.set_ylabel('$\Delta$ LWCF ($W/m^2$)',fontsize=12)
    if i == 0:
        if normalize is False:
            plt.title('Response to $CO_2$ Doubling',fontsize=12)
        else:
            plt.title('Response to $F_2$$_x$',fontsize=12)
    ax2.set_xticks(ticks)
    ax2.set_xticklabels(ticklabels,fontsize=12)
    ...
    ax3.set_ylabel('diff ($W/m^2$)',fontsize=12)
    if i == 0:
        ax3.set_title('Difference W.R.T. Present',fontsize=12)
    ...
    ax4.set_ylabel('$\Delta$ LWCF ($W/m^2$)',fontsize=12)
    if i == 0:
        if normalize is False:
            plt.title('Response to $CO_2$ Quadrupling',fontsize=12)
        else:
            plt.title('Response to $F_4$$_x$',fontsize=12)
    ax4.set_xticks(ticks)
    ax4.set_xticklabels(ticklabels,fontsize=12)
    #ax2.legend(loc='upper left')
    ax4.grid(True)
    ...
    ax5.set_ylabel('diff ($W/m^2$)',fontsize=12)
    if i == 0:
        ax5.set_title('Difference W.R.T. Present',fontsize=12)
    if i == 3:
        ax5.set_xlabel('Latitude (Deg N)',fontsize=12)
    ax5.set_xticks(ticks)
    ax5.set_xticklabels(ticklabels,fontsize=12)
    #ax3.legend(loc='upper left')
    ax5.grid(True)
...
'''
if normalize is False:
    fig.suptitle('Longwave Cloud Forcing',fontsize=24)
else:
...
if normalize is False:
    ...
    fig.suptitle('Zonal Mean Longwave Cloud Forcing Response to Equivalent Radiative Forcing',fontsize=16,y=1.05)


handles, labels = ax.get_legend_handles_labels()
fig.legend(handles,labels,loc = (0.28, 0.915), ncol=4 )
fig.tight_layout(pad=4)
plt.show()
if normalize is False:
    fig.savefig(figure_path+'ZM_LWCF_2x_Response.pdf',bbox_inches='tight')
else:
    fig.savefig(figure_path+'ZM_LWCF_n_Response.pdf',bbox_inches='tight')
###################################################################################################################################################
