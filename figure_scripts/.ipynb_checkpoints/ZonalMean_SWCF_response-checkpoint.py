...
for CASENAME in casenames:
    ...
    ax.set_ylabel('SWCF ($W/m^2$)',fontsize=12)
    ax.set_xticks(ticks)
    ...
    ax2.set_ylabel('$\Delta$ SWCF ($W/m^2$)',fontsize=12)
    if i == 0:
    ...
    ax4.set_ylabel('$\Delta$ SWCF ($W/m^2$)',fontsize=12)
    if i == 0:
        if normalize is False:
...
if normalize is False:
    ...
    fig.suptitle('Zonal Mean Shortwave Cloud Forcing Response to Equivalent Radiative Forcing',fontsize=16,y=1.05)
